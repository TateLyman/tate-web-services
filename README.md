...
- `agent-subscription-boundary-review.html` - paid private review for x402 and MPP flows where one paid agent request creates an account, subscription, API key, top-up, balance, or long-lived platform credential; listed on PayanAgent as `js73e4g7kq064b2jg0grafrrth8730wg`
- `agent-spend-guard.html` - paid private review for runaway agent cost controls, hard spend caps, retry budgets, usage evidence, and kill switches after the May 2026 managed-agent and agent-cost trend spike; listed on PayanAgent as `js7ax64pr8yfh8vjbkrjgynjgd873c4k`
- `x402-surface-check.html` - browser checker and CLI landing page for x402 manifests, OpenAPI specs, Streamable HTTP MCP `tools/list` catalogs, linked `discovery_url` catalogs, string and nested `discovery` links, `routes[]` catalogs, `resources[]` catalogs, endpoint-bearing `tools` maps, string-valued endpoint maps, raw resource URL strings, item catalogs, direct paid endpoints, pasted no-payment 402 challenges, framework-wrapped challenge bodies such as FastAPI-style `detail` objects, `schemes[]` challenge arrays, declared-price drift, cache-control posture, browser-readable challenge/session headers, common x402/MPP retry-header preflight, MPP descriptor-only 402 detection, accept-leg resource binding, timeout/expiry metadata, payment-metadata privacy checks for sensitive query fields, emails, token-like values, and prompt/private-context strings, optional strict-cache findings, optional strict-proof payment-identifier and signed offer/receipt checks, credential-like public registry URL params with redacted output, contextual guide links, browser origin/payment-header preflight blockers, MPP payment headers, x402 V2 requirements headers, MCP tool-name array handling, health/status routes, and legacy decimal x402 v1 challenges. Paid Agensi skill version: https://www.agensi.io/skill/dda183c1-4c47-4f39-a7f1-c4cec9004d20
- `x402-triage-mcp.html` - landing page for the official MCP Registry-listed `x402-triage-mcp` package and its read-only x402 surface / 402 Index tools
...
- `x402-triage-mcp` - official MCP Registry active server and npm package `x402-triage-mcp@0.1.1` for the public x402 triage / 402 Index watch workflow. Registry: https://registry.modelcontextprotocol.io/v0.1/servers?search=io.github.TateLyman%2Fx402-triage-mcp
- `agent-skill-trust-check` - public static pre-install scanner for SKILL.md, OpenClaw, Hermes, MCP, and paid agent-skill marketplace listings. Direct x402 API: https://the402.tateprograms.com/api/x402/skill-trust-check at $0.01 on Base mainnet USDC. Repo: https://github.com/TateLyman/agent-skill-trust-check. npm: https://www.npmjs.com/package/agent-skill-trust-check. AI Skill Store: https://aiskillstore.io/skills/be1a09eb-13c2-4bdf-ad00-8445e8394157. PayanAgent: `js76g8grh1rndrqpp77w87bzzn872jt6`. Paperclip: https://www.paperclipskills.com/api/v1/skills/@tateprograms/agent-skill-trust-check. Orkai: https://orkai.ai/skills/agent-skill-trust-check
- `security-remediation-plan` - AI Skill Store skill created from live unmet search demand for vulnerability remediation. Listing: https://aiskillstore.io/skills/8d5a2f7f-6217-4770-9aa4-3930020fefc4
